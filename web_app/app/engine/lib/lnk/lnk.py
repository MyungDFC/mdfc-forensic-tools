...
class TLNKFileParser:
    ...
    def parse_data(self):
        ...
        if (h.FileCreationTime.LowDateTime + h.FileCreationTime.HighDateTime) != 0:
            t = filetime_to_datetime(FileTime(h.FileCreationTime), 0)
            hinfo.append(addItem(RS_TargetFileCreateDT, t))
            # hinfo.append(addItem(RS_TargetFileCreateDT, FileTime(h.FileCreationTime)))  # TODO
        if (h.FileAccessTime.LowDateTime + h.FileAccessTime.HighDateTime) != 0:
            t = filetime_to_datetime(FileTime(h.FileAccessTime), 0)
            hinfo.append(addItem(RS_TargetFileAccessDT, t))
            # hinfo.append(addItem(RS_TargetFileAccessDT, FileTime(h.FileAccessTime)))  # TODO
        if (h.FileModifiedTime.LowDateTime + h.FileModifiedTime.HighDateTime) != 0:
            t = filetime_to_datetime(FileTime(h.FileModifiedTime), 0)
            hinfo.append(addItem(RS_TargetFileModifyDT, t))
            # hinfo.append(addItem(RS_TargetFileModifyDT, FileTime(h.FileModifiedTime)))  # TODO
        if h.FileSize != 0:
...
def lnk_parse_result(file, file_name):
